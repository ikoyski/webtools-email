pipeline {
    ...
    environment {
        ...
        DOCKERHUB_IMAGE = 'ikoyski/webtools-email:latest'        
        DEPLOYMENT_FILENAME = 'Deploy-webtools-email-private.yaml'
    }
    stages {
        stage('Git Stuff') {
            steps {
                script {
                	checkout scmGit(
                		...
                		extensions: [], 
                		userRemoteConfigs: [[url: 'https://github.com/ikoyski/webtools-email.git']]
                	)
                }
            }
        }
